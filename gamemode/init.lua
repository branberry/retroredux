...
SetGlobalFloat('EndTime', (CurTime() + 64))
end

function GM:SpawnTeamInfo()
  self.Spawns = {}
  self.TeamInfos = {}
  ...
  local spawns = ents.FindByClass(v.SpawnEnt)
   if not table.IsEmpty(spawns) then
      ...
      table.insert(self.TeamInfos, teamindex, {Score = 0})
...
function GM:FullGameUpdate(pl) -- Updates the naive client with round information that is obtained through networked strings.
  ...
  net.Start('nox_RoundStatus')
  net.WriteInt(self.RoundStatus, 8)
  net.WriteBool(false)
  net.Send(pl)
...
function GM:EndRound(WinningIndex, CameraLockData)
  ...
  self:SetRoundStatus(-1) -- Indicates the round has ended.
    if CameraLockData then
...
return self.RoundStatus end

function GM:SetRoundStatus(status, instant)
  ...
  if status == -1 then
    SetGlobalFloat('EndTime', CurTime() + GAMEMODE.EndRoundDuration)
  end
end
...
function GM:EndRoundThink()
  ...
  if self.LerpTimeInfo['ShouldLerp'] then
    self:TimeLerpTick()
  end

  if self.SendingHMs and self.NextHM < CurTime() then
      self:PostHonorableMention()
  ...
  if CurTime() >= GetGlobalFloat('EndTime', 64) then 
    self:RestartRound()
  end
end
...
concommand.Add('nox_EndRound', function(pl, cmd, args, argstbl) 
  if tonumber(argstbl[1]) then 
    gamemode.Call('EndRound', tonumber(argstbl[1])) 
  end
end)

concommand.Add('nox_SetEndTime', function(pl, cmd, args, argstbl) 
  if tonumber(argstbl[1]) then 
    SetGlobalFloat('EndTime', CurTime() + tonumber(argstbl[1])) 
  end
end)
...
net.Receive('nox_CastSpell', function(len, pl)
