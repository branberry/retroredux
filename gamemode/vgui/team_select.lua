...
local function ButtonPaint(self)
    ...
    local plys = team.GetPlayers(self.playingid)
    local texquad = {
...
function RemoveTeamSelect()
    GAMEMODE.PanelTeamSelect:Remove()
end

local function ButtonDoClickEvent(self)
    RunConsoleCommand('nox_teamswitch', tostring(self.playingid))
    RemoveTeamSelect()
end
...
function DrawTeamSelect()
    if GAMEMODE.PanelTeamSelect and GAMEMODE.PanelTeamSelect:IsValid() then RemoveTeamSelect()
    else

        GAMEMODE.PanelTeamSelect = vgui.Create('TeamSelect')
        GAMEMODE.PanelTeamSelect:MakePopup()

        local screens = math.min(1, ((ScrW() / 3840) + 0.5) ^ 2)
        local centx, centy = ScrW()/2, ScrH()/2
        DrawTeams(GAMEMODE.PanelTeamSelect, screens, centx, centy)
