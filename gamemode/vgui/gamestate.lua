local PANEL = {}
local objtext = 'Intermission'


local function PaintText(w, h)

  draw.SimpleText(objtext, 'DefaultFontSmall', w/2, h/3, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
  ...
  if #sectext < 2 then
    draw.SimpleText(tostring(minutes) .. ':0' .. tostring(math.Truncate(seconds)), 'DefaultFontMed', w/2, h/1.5, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    -- surface.DrawText(tostring(minutes) .. ':0' .. tostring(math.Truncate(seconds)))
    else
      draw.SimpleText(tostring(minutes) .. ':' .. tostring(math.Truncate(seconds)), 'DefaultFontMed', w/2, h/1.5, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
